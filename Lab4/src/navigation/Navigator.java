...
public class Navigator extends Thread implements Navigation {
	...
	public void travelToOrigin(double currentX, double currentY, double angle){
		currentDestination = new Vector(0, 0);
		currentPosition = new Vector(currentX, currentY);
		Vector difference = VectorOperations.subtract(currentDestination, currentPosition);
		double theta = difference.getAngle() - angle;
		turnTo(theta);
		motorController.travel(difference.getLength());
		turnTo(-theta);
	}
